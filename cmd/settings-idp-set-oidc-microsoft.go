...
// setMicrosoftIdpCmd represents the get command
var setMicrosoftIdpCmd = &cobra.Command{
	Use:   "microsoft",
	Short: "Set microsoft idp configuration",
...
func init() {
	setIdpCmd.AddCommand(setMicrosoftIdpCmd)

	initOutputFlags(setMicrosoftIdpCmd)
	initLoopControlFlags(setMicrosoftIdpCmd)
	initTenantFlags(setMicrosoftIdpCmd)

	initInputFlags(setMicrosoftIdpCmd, "idp",
		inputField{
			Name:            "Allowed Azure ADs",
			FlagName:        "allowed-azure-ads",
			FlagDescription: "List of allowed Azure Active Directory IDs",
			VarType:         "[]string",
			Mandatory:       true,
			DefaultValue:    []string{},
		})
